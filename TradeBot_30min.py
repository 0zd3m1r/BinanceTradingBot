                   ...
                   for line in lines:
                        ...
                        try:
                           ...
                           if last_ema52 > last_ema26 and previous_ema26 > previous_ema52:
                            ...
                            bot.sendMessage(chat_id=chatid, text=(shitemoji*1) + "SAT (TOP 200) 30 Dakikalık Grafik" + "\n" + symbol + "\n" + "Satış girilecek değer: "+str("{:.16f}".format(sellavg))+ "\n30 dk dilimdeki düşüş oranı: "+str("{0:.4%}".format(ratiodown))+ thumbsdown +"\nEMA'ya Uzaklık: "+str("{0:.4%}".format(ratioEMAdown))+emojiema + "\nHacim "+str("{:.1f}".format(volumeratio))+ " kat arttı!")
                            ...
                            bot.sendMessage(chat_id=chatid, text=(fireemoji*1)+ "AL (TOP 200) 30 Dakikalık Grafik" + "\n" + symbol + "\n" + "Alım girilecek değer: "+str("{:.16f}".format(buyavg))+ "\n30dk dilimdeki yükseliş oranı: "+str("{0:.4%}".format(ratioup))+ thumbsup +"\nEMA'ya Uzaklık: "+str("{0:.4%}".format(ratioEMAup))+emojiema+ "\nHacim "+str("{:.1f}".format(volumeratio))+ " kat arttı!")
                           count += 1
                        except: count+=1
